...
void i2c::enable_dma_last_bit(bool b){constexpr uint8_t cr2_last = 12; i2c_type::get()->cr2.bb_putbit<cr2_last>(b);}

void i2c::enable_dma_tx_stream(bool b)
{
   if (b){
...
void i2c::enable_dma_rx_stream(bool b)
{
   if (b){
...
{
    DMA1->HIFCR |= (1 << 5) ; // DMA1.Stream4 (TCIF)
}

void i2c::clear_dma_rx_stream_tcif()
{
...
void i2c::set_dma_rx_buffer(uint8_t * data, uint16_t numbytes)
{
...
void i2c::enable_error_interrupts(bool b)
{
   constexpr uint8_t cr2_error_bit = 8;
   i2c_type::get()->cr2.bb_putbit<cr2_error_bit>(b);
}
...
{
   constexpr uint8_t cr2_itbufen_bit = 10;
...
{
   pfn_dma_tx_handler = pfn_event;
}

void i2c::set_dma_rx_handler( void(*pfn_event)())
{
   pfn_dma_rx_handler = pfn_event;
}
